...
1. **Should `STEP_TO_TASK["refine"] = "refine.run"` be registered in Phase 1 at all, or defer to Phase 2?**
   ...
   - RESOLVED: Register in Phase 1 per CONTEXT, but add an explicit comment in `STEP_TO_TASK` documenting the "unused-until-Phase-2 reservation." Phase 1 tests must NOT submit jobs with `enable_refine=True` (the CFG-04 / CFG-06 tests stop at the `create_job` boundary without running the pipeline). Reflected in Plan 04 Task 2.
...
2. **What fields should the `RefineEditOp` for `op="delete"` carry beyond `target_note_id` + `rationale`?**
   ...
   - RESOLVED: Stick with Claude's Discretion for Phase 1 — target_note_id + rationale only. Phase 2 can add optional fields — schema stays backward-compatible because they'd be optional. Reflected in Plan 01 Task 1's RefineEditOp shape.
...
3. **Should the allowlist validator run only on `refine_model` or also on `refine_max_tokens` / `refine_web_search_max_uses` / `refine_max_retries` upper bounds?**
   ...
   - RESOLVED: Phase 1 — no upper-bound validator beyond Pydantic's type coercion. Phase 2 may introduce one if empirical testing shows over-large values cause silent Anthropic errors. Defer to Phase 2's empirical discretion. Reflected in Plan 03 Task 1 (only the model allowlist has a `model_validator`; other refine knobs are plain typed fields).
...
4. **Does the pre-commit hook need to run in CI, not just on developer machines?**
   ...
   - RESOLVED: Phase 1 scope is dev-machine only — create the hook + baseline. CI wiring (adding a `.github/workflows/*.yml` step) is out of scope for Phase 1's 5 success criteria. Flagged for a future ops phase if accidental-commit telemetry shows UI-path commits need coverage.
